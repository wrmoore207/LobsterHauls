...
def run_county_value_delta():
    ...
    for i, county in enumerate(unique_counties):
        ...
        axes[i].plot(county_data['YEAR'], county_data['value_yoy'], marker='o', linestyle='-')
        axes[i].set_title(county)
        axes[i].set_xlabel('Year')
        axes[i].set_ylabel('Value YoY (%)')
        axes[i].grid()

    plt.tight_layout()
    plt.savefig("Figs/CountyValueDelta.svg")
    plt.show()
